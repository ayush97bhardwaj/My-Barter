...
.post(function(req,res,next){
    ...
    User(req.body).validate(function(err){
        if(err) {
            ...
            // if(err.toString().indexOf('`phone` is required')!=-1)
            //     str+='<li class="form-text text-danger">Fill in your phone </li>';
            if(err.toString().indexOf('`phone` to be unique')!=-1)
                str+='<li class="form-text text-danger">phone is already registered </li>';
